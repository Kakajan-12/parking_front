...
import type { TariffCreate, TariffVisible, ValidationError } from "@/openapi/client";
...
interface IResponse {
    ...
    data?: TariffVisible | null;
}

export const createAction = async (values: TariffCreate): Promise<IResponse> => {
    ...
    try {
        const response = await fetchClient.carPark.tariffCreate({
            requestBody: values,
        });
