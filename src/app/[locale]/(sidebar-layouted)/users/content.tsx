...
function Content({ rows, page, limit, totalCount, search }: Props) {
    ...
    const handleDelete = async (value: string) => {
        ...
        try {
            const response = await userDeleteAction(value);
            if (response.status == 200) {
                toastUpdate(
                    toastId,
                    response.message ?? t("users-page.user-deleted-successfully"),
                    "success",
                );
                router.refresh();
            } else {
    ...
    const columns: ColumnDef<UserResponse>[] = [
        ...
        },
        {
            ...
            header: t("role"),
            cell: ({ row }) => {
                return <Badge variant="outline">{row.original.role.label}</Badge>;
            },
        },
        {
            accessorKey: "carPark",
            ...
            cell: ({ row }) => {
                ...
                if (!carPark) return "-";
                return <Badge variant="outline">{carPark.label}</Badge>;
            },
        },
        {
